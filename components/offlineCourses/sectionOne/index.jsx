...
const SectionOne = ({ time, students, courses }) => {
  return (
    <div className='w-full flex items-center justify-start flex-col p-0 before:w-4/12 relative before:absolute before:h-full before:-z-10 after:-z-10 before:left-0 before:top-0 before:bg-blueHamgo after:absolute after:h-full after:bg-white after:right-0 after:top-0 after:w-8/12 overflow-hidden'>
        <div className='w-full h-full z-50 flex items-stretch justify-center p-8 res4:p-3'>
            <div className='w-8/12 res6:w-full h-auto flex items-center justify-center flex-col z-40'>
                <div className='h-full flex items-start justify-center flex-col mt-20 transform translate-x-16 res6:translate-x-0'>
                    <h3 className='text-black33 font-extrabold text-4xl res3:text-2xl mb-8'>دوره های آفلاین همگو</h3>
                    <pre className='text-black33 text-sm res3:text-xs font-medium font-[IRANSans-Medium] text-start'>
                        {`
با دوره های آفلاین همگو بازار کار تو مشتته! \n
...
اینجا یه تیم برای رسیدن به اهدافت پشتته
                ...
                <div className='w-full flex items-center justify-center mt-20 translate-y-20  res6:translate-y-0'>
                    <div className='flex items-center justify-between flex-col w-48 m-4 res3:mx-0.5'>
                        ...
                        <p className='text-black33 text-sm text-center'>بیش از <span>{time?.toLocaleString('fa-IR')}</span> ساعت آموزش مجازی</p>
                    </div>
                    <div className='flex items-center justify-between flex-col w-48 m-4 res3:mx-0.5'>
                        <FaGraduationCap className='text-8xl res5:text-4xl my-3' />
                        <p className='text-black33 text-sm text-center'>بیش از <span>{students?.toLocaleString('fa-IR')}</span> دانش آموخته</p>
                    </div>
                    <div className='flex items-center justify-between flex-col w-48 m-4 res3:mx-0.5'>
                        <FaChalkboardTeacher className='text-8xl res5:text-4xl my-3' />
                        <p className='text-black33 text-sm text-center'>بالغ بر <span>{courses?.toLocaleString('fa-IR')}</span> دوره آنلاین</p>
                    </div>
                </div>
            </div>
